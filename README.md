...
The workflow of the programmer using this tool may be exemplified in the following manner:
   ...
   5. *Overwriting out-of-date source files with newer ones from temporary location is the programmer's responsibility*. Shell/Batch scripts provided by *CodeAugmentor* make this easier and even provide commands to list changed files (like git status), and view diffs of source files about to be changed (like git diff).
